...
export class ProfileModal extends InteractionHandler {
  ...
  public override async run(interaction: ModalSubmitInteraction) {
    ...
    const [alreadyExists] = await kil
      .select({ userId: songProfileSources.userId })
    ...
    if (alreadyExists) {
      ...
      playerLogger.info(
        `[Sync] ${interaction.user.tag} updated their Spotify profile to ${field}`,
      );
      return;
    } else {
      ...
      playerLogger.info(
        `[Sync] ${interaction.user.tag} set their Spotify profile to ${field}`,
      );
    }
