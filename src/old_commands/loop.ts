...
const execute = async (
  context: CommandContext<{ state?: keyof typeof LoopStates }>,
) => {
  const {
    member,
    input: { state },
  ...
  return setLoopState(member, state ? LoopStates[state] : undefined)
    .then((newState) => i18next.t('player.commands.loop.success', {
      ns: 'player',
      loop: newState,
    }))
    .catch((error) => getReadableException(error, context.guild));
};
