...
export const setupCommand = createCommand(
  ...
  async (context) => {
    ...
    return setupNewChannel(context.guild)
      .then((newChannel) => {
        newChannel.send(
          `${context.user.toString()} you can now use the bot in this channel.`,
        );
      })
      .catch((e) => {
        const readableException = getReadableException(e);
        context.user.send(
          `An error occured while setting up the bot: ${readableException}`,
        );
      });
  },
);
