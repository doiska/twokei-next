...
const execute = async (
  ...
  const [playlist] = await kil
    .select()
    .from(playlists)
    .where(
      and(eq(playlists.userId, user.id), ilike(playlists.playlistName, name)),
    );
  ...
  try {
    ...
    const trackTranslation = await t(
    ...
    const message = new MessageBuilder({
      embeds: [{ description: trackTranslation }],
    });
