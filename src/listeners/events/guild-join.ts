...
import { askLanguage } from '../../modules/tutorial/ask-language';

export const guildJoin = createEvent(Events.GuildCreate, async guild => {
  const { id, name: guildName, preferredLocale } = guild;
  ...
  adminUsableChannel.send('Hello! I\'m Twokei, a music bot. I\'m here to help you listen to music with your friends.').catch(() => {
    logger.warn(`Guild ${guildName} (${id}) has no usable channel for me :(`);
  });

  const message = await adminUsableChannel.send({
    embeds: [new EmbedBuilder().setDescription('Loading...')]
  });

  const lang = await askLanguage(message, guild);
  ...
  const embed = new EmbedBuilder()
    .setTitle(t('join_embed.title'))
    .setThumbnail(Twokei.user!.displayAvatarURL({ size: 2048 }) ?? '')
    .setDescription(t('join_embed.description', {
      playCommand,
  ...
  const introduction = await message.edit({ embeds: [embed], components: [row] });
