...
export const setupNewChannel = async (guild: Guild) => {
  ...
  if (!guild || !self || !Twokei.user?.id) {
    throw new FriendlyException('I can\'t setup the bot in this server, check if I have the correct permissions.');
  }
  ...
  if (currentChannel) {
    await guild.channels.fetch(currentChannel.channelId)
