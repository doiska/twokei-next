...
export async function createSongProfileEmbed(
  ...
  const sourcesWithEmojis =
    profile?.sources.map((s) => {
      ...
      }](https://open.spotify.com/user/${s.sourceUrl})`;
    }) ?? [];
  ...
  const profileEmbed = new EmbedBuilder()
    ...
    .setDescription(
      [isPremium && t("profile:embed.premium"), title, description]
        .filter(Boolean)
        .join("\n"),
    )
    .setFields([
      {
        ...
        value: sourcesWithEmojis.length
          ? sourcesWithEmojis.join("\n")
          : "None yet :(",
        inline: true,
      },
      {
  ...
  if (isMyProfile) {
    ...
    const isLiked = await container.profiles.actions.isLiked(
      requester.id,
      target.id,
    );
